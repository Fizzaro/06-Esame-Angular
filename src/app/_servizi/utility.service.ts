...
export class UtilityService {
   ...
   * @param sale Stringa inviata dal server
   * @returns stringa cifrata con sha512
   */
  static cifraPssw(password: string, sale: string): string {
    return sha512(password + sale)
  }

  /**
   * Funzione per decifrare il token JWT
   * @param token Stringa da decifrare
   * @returns any
   */
  static decifraToken(token: string): any {
    try {
  ...
  static generaCodFisc(nome: string, cognome: string, sesso: string, dataNascita: string, localita: string): string {
    ...
    var anno = data.getFullYear().toString().slice(-2); //PRENDI LE ULTIME DUE CIFRE

    ///####################MESE E GIORNO##############################///
    var cod_mese = ['A', 'B', 'C', 'D', 'E', 'H', 'L', 'M', 'P', 'R', 'S', 'T'];
    var mese: string = (data.getMonth() + 1).toString(); //perché getmonth parte da 0
    var giornoNumb: number = data.getDate();
    var giorno:string=''
    ...
    if (consonanti.length < 3) {
      //prendo le consonanti a posizione 0,1 e la prima vocale
      consonanti_prese_cognome = [consonanti[0], consonanti[1], vocali[0]].toString();
    }

    else {
      //prendo le consonanti a posizione 0,1,2
      consonanti_prese_cognome = [consonanti[0], consonanti[1], consonanti[2]].toString();
    }

    //SE LE CONSONANTI SONO 1
    if (consonanti.length == 1) {
      consonanti_prese_cognome = [consonanti[0], vocali[0], vocali[1]].toString();
    }
    ...
    switch (mese) {
      case '1': mese = cod_mese[0];
        break;
      case '2': mese = cod_mese[1];
        break;
      case '3': mese = cod_mese[2];
        break;
      case '4': mese = cod_mese[3];
        break;
      case '5': mese = cod_mese[4];
        break;
      case '6': mese = cod_mese[5];
        break;
      case '7': mese = cod_mese[6];
        break;
      case '8': mese = cod_mese[7];
        break;
      case '9': mese = cod_mese[8];
        break;
      case '10': mese = cod_mese[9];
        break;
      case '11': mese = cod_mese[10];
        break;
      case '12': mese = cod_mese[11];
        break;
    }
    //# CASE DEI GIORNI 
    switch (giornoNumb) {
      ...
      case 26: case 27: case 28: case 29: case 30: case 31:
        ...
        var giorno: string=giornoNumb.toString()
    ...
    var localita_presa= this.cod_citta[this.provinciaEl.indexOf(localita.toUpperCase())]
